...
# <center>
# <table><tr>
# <td> <img src="img/prppg2.png" alt="Drawing" style="width: 800px;"/> </td>
# </tr></table>
# </center>
# 
# <center>
...
Formas convenientes de sintetizar dados é por meio de tabelas de frequência e gráficos.
...
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=FALSE, echo = TRUE}
medidas <- data.frame(minimo = quantile(quest$tc_anoFimGrad)[1],
                      quart1 = quantile(quest$tc_anoFimGrad)[2],
                      media = mean(quest$tc_anoFimGrad),
                      mediana = quantile(quest$tc_anoFimGrad)[3],
                      moda = names(sort(table(quest$tc_anoFimGrad), 
                                        decreasing = TRUE)[1]),
                      quart3 = quantile(quest$tc_anoFimGrad)[4],
                      max = quantile(quest$tc_anoFimGrad)[5])

row.names(medidas) <- NULL

pander(medidas)

disp <- data.frame(amplitude = diff(range(quest$tc_anoFimGrad)),
                   variancia = var(quest$tc_anoFimGrad),
                   desv_pad = sd(quest$tc_anoFimGrad),
                   coef_var = 100*sd(quest$tc_anoFimGrad)/mean(quest$tc_anoFimGrad))
...
barplot(loc_ord$freq, col = '#BEF781', 
        ...
        xlim = c(0, max(loc_ord$freq + 10)),
        main = "Graduação",
        las = 2)

text(y = as.vector(barplot(loc_ord$freq, plot = FALSE)), 
     x = as.vector(loc_ord$freq) + 5,
...
prog <- data.frame(niveis = names(fr_prog),
                   #freq = as.vector(fa_prog),
                   freq_r = as.vector(fr_prog),
                   ano = rep(2020,length(fr_prog))) # unindo as informações
...
prog2 <- data.frame(niveis = names(fr_prog),
                   #freq = as.vector(fa_prog),
                   freq_r = as.vector(fr_prog),
                   ano = rep(2019,length(fr_prog))) # unindo as informações


prog[8:14,] <- prog2
prog$ano <- as.factor(prog$ano)
...
ggplot(prog, 
       aes(x=niveis, y=freq_r, fill=ano)) + 
  geom_bar(stat="identity", position = "dodge",alpha = 0.8, col = 1) +
  xlab("") + ylab("Frequência relativa") + theme_classic()+
  coord_flip() + ggtitle("Comparativo das Turmas")
```
...
pander:::pander(cat( "Note que pela 1a pergunta do questionário, temos ", 
                     qtddFimMestrado, 
                     " alunos que já terminaram o Mestrado. Pelas respostas válidas, temos ",
                     length(tempoMestValido), "alunos que concluíram o Mestrado."))
...
barplot(lmest_ord$freq, col = '#D0A9F5', 
        ...
        xlim = c(0, max(lmest_ord$freq + 10)),
...
barplot(bolsa_ord$freq, 
        ...
        ylab = 'Frequência',
        xlab = '',
        ylim = c(0, max(bolsa_ord$freq)+10)
        #horiz = T,
        #las=2,
)
...
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=FALSE, echo = TRUE, fig.height= 10}

soft21 <- arrange(soft2, soft2$freq)

barplot(soft21$freq, horiz = TRUE, 
        names.arg = soft21$niveis,
        col = rainbow(length(unique(soft21$niveis))),
        main = "Softwares Estatísticos Usados",
        xlim = c(0, max(soft21$freq + 10)), 
        cex.names=0.8, las = 2)
...
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=FALSE, echo = TRUE}
medio1$niveis <- c('Particular', 'Púbica', 'Ambos')
...
medidas <- data.frame(minimo = quantile(quest$tc_peso)[1],
                      quart1 = quantile(quest$tc_peso)[2],
                      media = mean(quest$tc_peso),
                      mediana = quantile(quest$tc_peso)[3],
                      moda = names(sort(table(quest$tc_peso), 
                                        decreasing = TRUE)[1]),
                      quart3 = quantile(quest$tc_peso)[4],
                      max = quantile(quest$tc_peso)[5])

row.names(medidas) <- NULL

pander(medidas)

disp <- data.frame(amplitude = diff(range(quest$tc_peso)),
                   variancia = var(quest$tc_peso),
                   desv_pad = sd(quest$tc_peso),
                   coef_var = 100*sd(quest$tc_peso)/mean(quest$tc_peso))
...
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=FALSE, echo = TRUE}
h <- hist(quest$tc_peso[which(quest$tc_peso > 0)],
          main = "Histograma do Peso", xlab = "Peso",
          ylab = "Densidade", col = "cornflowerblue", 
          probability = FALSE)

y <- density(quest$tc_peso[which(quest$tc_peso > 0)])
...
breaks <- c(0, 16, 17, 18.5, 25, 30, 35, 40, range(quest$tc_IMC, na.rm = TRUE)[2])
classes <- cut(quest$tc_IMC, breaks = breaks, 
...
medidas <- data.frame(minimo = quantile(quest$tc_IMC)[1],
                      quart1 = quantile(quest$tc_IMC)[2],
                      media = mean(quest$tc_IMC),
                      mediana = quantile(quest$tc_IMC)[3],
                      moda = names(sort(table(quest$tc_IMC), 
                                        decreasing = TRUE)[1]),
                      quart3 = quantile(quest$tc_IMC)[4],
                      max = quantile(quest$tc_IMC)[5])

row.names(medidas) <- NULL

pander(medidas)

disp <- data.frame(amplitude = diff(range(quest$tc_IMC)),
                   variancia = var(quest$tc_IMC),
                   desv_pad = sd(quest$tc_IMC),
                   coef_var = 100*sd(quest$tc_IMC)/mean(quest$tc_IMC))
...
```{r, comment=FALSE, message=FALSE, fig.align='center', warning=FALSE, echo = TRUE}
h <- hist(quest$tc_IMC, breaks = breaks, main = "Histograma do IMC", xlab = "IMC", 
          ylab = "Densidade", col = "yellow", freq = FALSE, ylim = c(0, 0.10))
lines(density(quest$tc_IMC[which(quest$tc_IMC > 0)]), lwd = 3)
rug(quest$tc_IMC[which(quest$tc_IMC > 0)])
xx <- seq(0,60, len=200)
yy <- dnorm(xx, mean=mean(quest$tc_IMC[which(quest$tc_IMC > 0)], na.rm=T), sd=sd(quest$tc_IMC[which(quest$tc_IMC > 0)]))
...
barplot(mor_ord$freq, col = '#0B243B', 
        ...
        xlim = c(0, max(mor_ord$freq + 10)),
        main = "Moradia",
        las = 2)

text(y = as.vector(barplot(mor_ord$freq, plot = FALSE)), 
     x = as.vector(mor_ord$freq) + 5,
...
barplot(origem_ord$freq, 
        ...
        ylab = 'Frequência',
        xlab = '',
        ylim = c(0, max(origem_ord$freq)+10)
        #horiz = T,
        #las=2,
)
...
barplot(habit_ord$freq, col = '#F3F781', 
        ...
        xlim = c(0, max(habit_ord$freq + 10)),
        main = "Habilitação",
        las = 2)

text(y = as.vector(barplot(habit_ord$freq, plot = FALSE)), 
     x = as.vector(habit_ord$freq) + 5,
...
barplot(transp_ord$freq, 
        ...
        ylab = 'Frequência',
        xlab = '',
        ylim = c(0, max(transp_ord$freq)+10)
        #horiz = T,
        #las=2,
)
...
#### Foto 1 {.tabset .tabset-fade .tabset-pills}
...
hist(idade_prof1,col = "#CEF6F5", main = 'Idade professor')
...
#### Foto 2 {.tabset .tabset-fade .tabset-pills}

##### Tabela 1

```{r, comment=FALSE, message=FALSE, fig.align='center', warning=FALSE, echo = TRUE}
idade_prof2 <- quest$tc_idadeProf[175:nrow(quest)]

h <- hist(idade_prof2, plot = FALSE) #histograma
...
idade_prof22 <- quest[174:nrow(quest),]

medidas <- data.frame(minimo = quantile(idade_prof22$tc_idadeProf)[1],
                      quart1 = quantile(idade_prof22$tc_idadeProf)[2],
                      media = mean(idade_prof22$tc_idadeProf),
                      mediana = quantile(idade_prof22$tc_idadeProf)[3],
                      moda = names(sort(table(idade_prof22$tc_idadeProf), 
                                        decreasing = TRUE)[1]),
                      quart3 = quantile(idade_prof22$tc_idadeProf)[4],
                      max = quantile(idade_prof22$tc_idadeProf)[5])

row.names(medidas) <- NULL

pander(medidas)

disp <- data.frame(amplitude = diff(range(idade_prof22$tc_idadeProf)),
                   variancia = var(idade_prof22$tc_idadeProf),
                   desv_pad = sd(idade_prof22$tc_idadeProf),
                   coef_var = 100*sd(idade_prof22$tc_idadeProf)/mean(idade_prof22$tc_idadeProf))
...
hist(idade_prof2,col = "#CEF6F5", main = 'Idade professor')
